...
## Client initialization and dataset loading

The user must initialize the SWS client before selecting and loading a dataset.

![Client initialisation](./images/datasets_initialise.png){#fig-initialisation-and-datasets width="50%"}
...
![Dataset summary](./images/data_summary_loaded_dataset.png){#fig-dataset-summary width="50%"}
...
![Filters & Aggregation page](./images/filters_aggregation_page.png){#fig-filters-aggregation-page width="100%"}
...
![Geographical area filter](./images/geographical_area_filter.png){#fig-geographical-filter width="100%"}

![Asfis filter](./images/asfis_filter.png){#fig-asfis-filter width="100%"}

![Fishing area filter](./images/fishing_area_filter.png){#fig-fishing-area-filter width="100%"}
...
![Observation flag filter](./images/observation_flag_filter.png){#fig-observation-flag-filter width="100%"}
...
#| tbl-cap: "Illustrative cross-classification of species"
#| echo: false
...
![Aggregated total graph](./images/aggregated_total_graph.png){#fig-aggregated-total-graph width="100%"}
...
![Stacked bar chart](./images/stacked_bar_chart.png){#fig-stacked-bar-chart width="100%"}
...
![Relative stacked bar chart](./images/relative_stacked_bar_chart.png){#fig-relative-stacked-bar-chart width="100%"}
...
![Component time series](./images/component_time_series.png){#fig-component-time-series width="100%"}
...
![Treemap chart](./images/treemap_chart.png){#fig-treemap-chart width="100%"}
...
## Eligibility and required sequence
...
![Tagged dataset upload](./images/tagged_dataset_upload.png){#fig-tagged-dataset-upload width="100%"}
...
![All comparison rows](./images/all_comparison_rows.png){#fig-all-comparison-rows width="100%"}

![Differences only table](./images/differences_only_no_differences.png){#fig-differences-only-table width="100%"}
...
![Value outlier metric](./images/distribution_by_year_asfis.png){#fig-value-outlier-metric width="50%"}
...
![Year-on-year absolute change selector](./images/largest_records_absolute_change.png){#fig-year-on-year-metric width="50%"}
...
![Largest records table](./images/largest_records_data.png){#fig-largest-records-data width="50%"}
...
The **Number of top records to display** control appears when the **Largest / strongest records** tab is selected.
 The user can choose to display between **10 and 200 records**, in increments of 10. The default is **50 records**.
...
2. Select the required geographical areas in the geographical-area filter.
3. Set **Geographical area aggregation** to **Combine all filtered values into one output**.
4. In the species filter, select ISSCAAP group `1501`.
5. Set **Species aggregation** to **Keep filtered codes separate**.
...
technical_functions <- data.frame(
  ...
  Responsibility = c(
    "Standardize the Value column, convert it to numeric and remove rows without a valid numerical value",
    "Check that the dataset contains the required columns and identify missing optional columns",
    "Interpret tree selections, apply the hierarchy selection rule and, where required, include descendants",
    "Apply the active year and dimension filters to the selected dataset",
    "Create separate outputs for the selected filter groups and assign the remaining categories to Other",
    "Create outputs for the direct children of the selected hierarchy parents and assign the remaining categories to Other",
    "Create one custom aggregation output and assign the remaining categories to Other",
    "Aggregate one dimension by summing values according to the aggregation mapping and calculating aggregate flags",
    "Apply the selected aggregation settings across dimensions and optionally aggregate years or preserve observation-flag groups",
    "Match current and comparison results, calculate differences and assign comparison statuses",
    "Prepare Value, absolute-change or percentage-change metrics for outlier analysis"
  )
)
...
[Open the Shiny Application](https://sws.qa.fao.org/tasks/trigger/a12c26d0-6297-4bd5-a26c-82909115b124)

The GitHub repository:  
[Open the GitHub repository](https://github.com/un-fao/sws-stat-fisheries-aggregation)
